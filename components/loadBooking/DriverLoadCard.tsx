...
import {
  ...
  FaUsers, FaFlag, FaChevronRight, FaChevronLeft, FaCheckCircle, FaTimes, FaWhatsapp, FaPhoneAlt
} from "react-icons/fa";
  ...
  return (
    <>
      <motion.div
        ...
        className={`relative bg-gradient-to-br from-gray-900 via-gray-900 to-gray-800 border rounded-2xl overflow-hidden shadow-xl transition-all duration-300 group mb-12 lg:mb-8 ${isInactive
        ...
        <div className="p-4">
          ...
          <div className="bg-white/5 border border-white/5 rounded-xl p-2.5 mb-3 grid grid-cols-3 gap-2">
            ...
            <div className="min-w-0">
              ...
              <p className="text-white text-[10px] font-bold capitalize truncate">{booking.vehicleColor}</p>
            </div>
            <div className="min-w-0">
              <p className="text-[7px] font-black text-gray-600 uppercase tracking-widest mb-0.5">Plate</p>
              <p className="text-white text-[10px] font-bold font-mono truncate">{booking.vehiclePlate}</p>
            </div>
          </div>
            ...
            <div className="space-y-3 pb-3 border-b border-white/5">
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1">
                  <span className="text-xs font-black text-red-300 uppercase tracking-widest mr-1.5">Going to:</span>
                  <span className="text-white text-xs font-bold leading-tight">{booking.destination}</span>
                </div>
              </div>
              <div className="flex items-start gap-2">
                <FaMapMarkerAlt className="text-green-400 shrink-0 mt-0.5" size={10} />
                <div className="flex-1">
                  <span className="text-xs font-black text-green-300 uppercase tracking-widest mr-1.5">Meet at:</span>
                  <span className="text-white text-xs font-bold leading-tight">{booking.meetingPoint}</span>
                </div>
              </div>
            </div>
          ...
          <div className="flex gap-2">
            <button
              onClick={() => {
                if (hasBooked) onFlag(booking);
              }}
              disabled={!hasBooked}
              className={`w-9 h-9 flex items-center justify-center border rounded-xl transition-colors shrink-0 ${
                hasBooked
              ...
              title={hasBooked ? "Flag Driver" : "Book a seat first to flag"}
            >
              <FaFlag size={11} />
            </button>
            ...
            {hasBooked ? (
              <a
                href={booking.whatsappPreferred 
                  ? `https://wa.me/${booking.driverPhone?.replace(/\D/g, '')}?text=Hi%20${encodeURIComponent(booking.driverFirstName || booking.driverName)},%20I%20am%20interested%20in%20your%20ride%20to%20${encodeURIComponent(booking.destination)}`
                  : `tel:${booking.driverPhone}`
                }
                target={booking.whatsappPreferred ? "_blank" : "_self"}
                rel="noopener noreferrer"
                className={`w-9 h-9 flex items-center justify-center border rounded-xl transition-colors shrink-0 ${
                  booking.whatsappPreferred 
                ...
                title={booking.whatsappPreferred ? "WhatsApp Driver" : "Call Driver"}
              >
                {booking.whatsappPreferred ? <FaWhatsapp size={14} /> : <FaPhoneAlt size={11} />}
              </a>
            ) : (
              <button
                disabled
                className="w-9 h-9 flex items-center justify-center rounded-xl transition-colors shrink-0 bg-gray-800/50 border border-gray-700/50 text-gray-600 cursor-not-allowed"
                title="Book a seat first to contact driver"
              >
                {booking.whatsappPreferred ? <FaWhatsapp size={14} /> : <FaPhoneAlt size={11} />}
              </button>
            )}
      ...
      <AnimatePresence>
        {showGallery && galleryImages.length > 0 && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[999] bg-black/95 backdrop-blur-xl flex items-center justify-center p-2 md:p-10"
          ...
          >
            ...
            >
              <div className="relative w-full h-[60vh] md:h-[75vh] rounded-xl overflow-hidden shadow-2xl border border-white/10 bg-gray-900">
                <motion.img
                  key={currentImgIndex}
                ...
                {/* Image Label Badge */}
                <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-black/60 backdrop-blur-md px-3 md:px-6 py-2 rounded-full border border-white/10">
